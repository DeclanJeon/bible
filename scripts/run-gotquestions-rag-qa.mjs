...
const okfQaFixturePath = path.join(ROOT, "qa", "gotquestions-okf-cases.generated.json");
...
try {
  const okfCases = JSON.parse(await readFile(okfQaFixturePath, "utf8")).filter((row) => row.surface === "rag");
  fixtures = [...fixtures, ...okfCases];
} catch {
  // OKF QA cases are generated by `npm run okf:gotquestions:qa-cases`.
}
